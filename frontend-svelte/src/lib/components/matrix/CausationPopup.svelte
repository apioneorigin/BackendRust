<script lang="ts">
	/**
	 * CausationPopup - Select row dimensions for the transformation matrix
	 *
	 * Features:
	 * - Document tabs (3 initial + ability to add more)
	 * - Max 10 row options to choose from per tab
	...
	$: canSubmit = currentTab && currentTab.selectedCausations.length === 5;
	$: canGenerateMore = currentTab && currentTab.causationOptions.length < 10;
	$: selectedCount = currentTab?.selectedCausations.length || 0;
	...
	function handleToggleOption(optionId: string) {
		if (currentTab) {
			matrix.toggleCausationSelection(currentTab.id, optionId);
		}
	}

	async function handleGenerateMore() {
		if (currentTab && canGenerateMore) {
			await matrix.generateMoreCausationOptions(currentTab.id);
		}
	}

	function handleSubmit() {
		if (currentTab && canSubmit) {
			const success = matrix.submitCausationSelection(currentTab.id);
			if (success) {
...
{#if open}
	...
	>
		<div
			class="causation-popup"
			on:click|stopPropagation
			on:keydown|stopPropagation
			role="dialog"
			aria-modal="true"
			aria-labelledby="causation-title"
		>
			<div class="popup-header">
				<h3 id="causation-title">Select Causation Dimensions</h3>
			...
			<div class="popup-body">
				<div class="selection-info">
					...
					<span class="selection-hint">Select exactly 5 causation dimensions</span>
				</div>

				{#if currentTab}
					<div class="options-list">
						{#each currentTab.causationOptions as option (option.id)}
							{@const isSelected = currentTab.selectedCausations.includes(option.id)}
					...
					{#if canGenerateMore}
						...
						>
							...
							{:else}
								...
								<span>Generate More Options ({currentTab.causationOptions.length}/10)</span>
							{/if}
						</button>
					{:else}
