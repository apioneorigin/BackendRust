...
export interface ArticulatedInsight {
	...
	your_truth: string;          // Recognition of user's reality + future detection tool
...
// Articulated Outcome structure based on Outcomes articulation logic.pdf
// 3-component structure: THE ARC → THE LANDSCAPE → THE ANCHOR (160-250 words total)
// Physics: PROJECTION (traces forces forward to resting states)
export interface ArticulatedOutcome {
	// Outcome title (max 10 words) - displayed in popup header
	title: string;

	// THE ARC (80-120 words): Force → inflection → resolution
	the_arc: string;                          // Progressive present tense, time-lapse, inside domain
	the_arc_destination: string;              // Bold destination (specific, not universal)

	// THE LANDSCAPE (50-80 words): Position → fork → state description
	the_landscape: string;                    // Current position + conditional fork
	the_landscape_operating_reality: string;  // Bold operating reality

	// THE ANCHOR (30-50 words): Name + signal + identity
	the_anchor_name: string;                  // Destination name, 2-5 words
	the_anchor_signal: string;                // Observable marker of arrival
	the_anchor_identity: string;              // Bold positional identity
}
...
export interface ColumnOption {
	...
	label: string;
	insight_title?: string;
	articulated_insight?: ArticulatedInsight;    // Legacy/fallback for existing data
	articulated_outcome?: ArticulatedOutcome;    // New outcome format for columns
}
