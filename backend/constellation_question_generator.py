...
Backend logic for deciding WHICH operators to target in the mandatory follow-up question.
Question content is generated by LLM (context-specific), not hardcoded archetypes.

ARCHITECTURE:
- Question generation is part of LLM Call 2 structured output (no separate LLM call)
...
- Question is conditionally included in Call 2 based on validation logic
...
- Exactly ONE question per response, always
- Backend selects target operators, LLM generates the question
- NEVER ask 2, 3, 4, or more questions — but NEVER skip either
"""
...
class ConstellationQuestionGenerator:
    ...
    def get_question_context(
        ...
        One mandatory question per response — no gating.
        ...
        Returns:
            Dict with question generation context (always returns, never None)
        """
